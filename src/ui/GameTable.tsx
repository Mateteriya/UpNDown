...
import type { AIDifficulty, DealResult, GameState, PlayerCount } from '../game/GameEngine';
import {
  createGame,
  createGameOnline,
  startDeal,
...
import {
  ...
  DEAL_RESULTS_COLLAPSING_MS as DEAL_RESULTS_COLLAPSING_MS_SHARED,
  DEAL_RESULTS_TOTALS_ACCENT_MS as DEAL_RESULTS_TOTALS_ACCENT_MS_SHARED,
  LAST_TRICK_CARDS_PAUSE_MS as LAST_TRICK_CARDS_PAUSE_MS_SHARED,
  LAST_TRICK_COLLECT_MS as LAST_TRICK_COLLECT_MS_SHARED,
  LAST_TRICK_REVEAL_MS as LAST_TRICK_REVEAL_MS_SHARED,
  LAST_TRICK_WINNER_PAUSE_MS as LAST_TRICK_WINNER_PAUSE_MS_SHARED,
  trickCompleteHoldMs,
...
/**
 * L-рамка (горизонталь 9 + вертикальный «хвост») — только если все карты
 * не влезают в один ряд при *комфортном* нахлёсте (как у 9 карт), без укладки стопкой.
 * Иначе на широком «мобильном» DevTools (растянутый device mode) хвост включался
 * зря по одному лишь `handLen >= 10` и прятался под столом.
 */
function mobileHandNeedsLFrame(vw: number, handLen: number): boolean {
  if (handLen < MOBILE_L_HAND_FROM_LEN) return false;
  const m9 = getMobileNineCardHandLayout(vw, Math.min(handLen, 9));
  const inset = mobileSouthStripInsetPx(vw);
  const gutter = inset * 4 + MOBILE_HAND_FRAME_EXTRA_INLINE_PX * 2 + 2 + MOBILE_TABLE_INNER_PAD_X_PX;
  const inner = Math.max(48, vw - gutter);
  const slotOuter = MOBILE_HAND_CARD_BODY_W + 2 * m9.slotPadding;
  const comfortO = Math.min(
    Math.max(m9.overlapPx, MOBILE_HAND_9_OVERLAP_BASE_PX),
...
/**
 * Южная ручка short-VH: 10×3 кружка. Три независимых «радужных» набора: соседи в ряду не повторяют оттенок,
 * ряды не затухают друг от друга (нет «второй ряд = тусклее»), столбцы специально смещены — без вертикальных пар.
 */
const MOBILE_SOUTH_PULL_TAB_DOT_COLORS_ROW1: readonly string[] = [
  '#ff0080',
  '#ff9500',
  '#ffea00',
  '#7cff00',
...
const MOBILE_SOUTH_PULL_TAB_DOT_COLORS_ROW2: readonly string[] = [
  '#22d3ee',
  '#e11d48',
  '#bef264',
  '#f97316',
...
const MOBILE_SOUTH_PULL_TAB_DOT_COLORS_ROW3: readonly string[] = [
  '#f472b6',
  '#39ff14',
  '#06b6d4',
  '#fcd34d',
...
interface GameTableProps {
  gameId: number;
  /** Число игроков для новой офлайн-партии (если нет сохранения). */
  offlinePlayerCount?: PlayerCount;
...
const LAST_TRICK_REVEAL_MS = LAST_TRICK_REVEAL_MS_SHARED;
/** Пауза с картами на столе (ещё без оверлея). */
const LAST_TRICK_CARDS_PAUSE_MS = LAST_TRICK_CARDS_PAUSE_MS_SHARED;
/** Длительность полёта карт к слоту победителя. */
const LAST_TRICK_COLLECT_MS = LAST_TRICK_COLLECT_MS_SHARED;
/** Пауза с миганием панельки взявшего взятку (+ старт оверлея). */
const LAST_TRICK_WINNER_PAUSE_MS = LAST_TRICK_WINNER_PAUSE_MS_SHARED;
/** Пауза только на умные «Итого» (running-total) до начала схлопывания. */
const DEAL_RESULTS_TOTALS_ACCENT_MS = DEAL_RESULTS_TOTALS_ACCENT_MS_SHARED;
/** Длительность фазы «таблица сворачивается в кнопку» */
const DEAL_RESULTS_COLLAPSING_MS = DEAL_RESULTS_COLLAPSING_MS_SHARED;
/** Пауза до следующей раздачи (offline / v1 host); v2 сервер = DEAL_COMPLETE_HOLD_MS. */
const TRICK_PAUSE_MS = DEAL_COMPLETE_HOLD_MS;
    ...
    if (playerCount === 3) {
      ...
      switch (playerIdx) {
        case 2:
          return {
            ...mobileBase,
            transform: `translate(calc(-50% - ${x} + ${gridX}), calc(-50% - ${y}))`,
          };
        case 1:
          return {
            ...mobileBase,
        ...
        case 0:
          return {
            ...mobileBase,
            transform: `translate(calc(-50% + ${gridX}), calc(-50% + ${y}))`,
          };
        default:
          return {
            ...mobileBase,
  ...
  if (playerCount === 3) {
    ...
    switch (playerIdx) {
      case 2:
        return {
          ...base,
          left: `calc(${offsetWestEast} + ${clusterShift})`,
          top: '50%',
      ...
      case 1:
        return {
          ...base,
          right: `calc(${offsetWestEast} - ${clusterShift})`,
          top: '50%',
          transform: 'translateY(-50%)',
        };
      case 0:
        return {
          ...
          bottom: offsetEdge,
          left: '50%',
          transform: `translateX(calc(-50% + ${clusterShift}))`,
        };
      default:
        return {
          ...base,
...
function getCurrentTrickLeaderIndex(state: GameState): number | null {
  ...
  return getTrickPlayerIndex(state.trickLeaderIndex, winnerOffset, n);
}
...
function getTrickSlotTransform(
  ...
  if (isMobileOrTablet) {
    ...
    if (playerCount === 3) {
      ...
      switch (playerIdx) {
        case 2:
          return `translate(calc(-50% - ${x} + ${gridX}), calc(-50% - ${y}))`;
        case 1:
          return `translate(calc(-50% + ${x} + ${gridX}), calc(-50% - ${y}))`;
        case 0:
          return `translate(calc(-50% + ${gridX}), calc(-50% + ${y}))`;
        default:
          return `translate(calc(-50% + ${gridX}), calc(-50% + ${y}))`;
      }
    }
  ...
  if (playerCount === 3) {
    ...
    switch (playerIdx) {
      ...
      case 0:
        return `translateX(calc(-50% + ${clusterShift}))`;
      default:
        return `translateX(calc(-50% + ${clusterShift}))`;
    }
  }
...
const TRICK_COLLECT_EASE = 'cubic-bezier(0.22, 0.61, 0.36, 1)';
...
function getTrickCollectSlotStyle(
  ...
  return {
    ...
    alignItems: 'center',
    pointerEvents: 'none',
    transform: getTrickSlotTransform(slotIdx, isMobileOrTablet, false, playerCount),
    /* Transition всегда — иначе смена transition+transform в одном кадре глотает полёт. */
    transition: `transform ${cardCollectMs}ms ${TRICK_COLLECT_EASE}`,
  };
}
...
function useIsTabletPcShell() {
  const [match, setMatch] = useState(
    () => typeof window !== 'undefined' && window.matchMedia(TABLET_PC_SHELL_MQ).matches,
  );
  useEffect(() => {
    const mq = window.matchMedia(TABLET_PC_SHELL_MQ);
    const handler = () => setMatch(mq.matches);
    mq.addEventListener('change', handler);
    return () => mq.removeEventListener('change', handler);
  }, []);
...
export default function GameTable({ gameId, offlinePlayerCount = 4, playerDisplayName, playerAvatarDataUrl, playerAvatarBgColor, onExit, onNewGame, onOpenProfileModal, onSaveAvatar, onPhotoCaptured, onSaveDisplayName }: GameTableProps) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      e.preventDefault();
      const now = Date.now();
      if (now - pcScaleWheelLockRef.current < 85) return;
      pcScaleWheelLockRef.current = now;
      if (e.deltaY < 0) bumpPcHandScale(1);
      else if (e.deltaY > 0) bumpPcHandScale(-1);
    };
  ...
  const [lastTrickCollectingPhase, setLastTrickCollectingPhase] = useState<
    'idle' | 'reveal' | 'slots' | 'winner' | 'totals-accent' | 'collapsing' | 'button'
  >('idle');
  ...
  useEffect(() => {
    ...
    if (
      isMobile ||
      !state ||
      getDealType(state.dealNumber, playerCountOf(state)) !== 'dark' ||
      dealContractStats.allBidsPlaced
    ) {
  ...
  useEffect(() => {
    if (isWaitingInRoom) {
      ...
      if (immersiveRevealPulseTimeoutRef.current != null) {
        window.clearTimeout(immersiveRevealPulseTimeoutRef.current);
        immersiveRevealPulseTimeoutRef.current = null;
      }
      setMobileShortImmersiveWelcomeOpen(false);
      setImmersiveRevealBtnPulse(false);
      return;
    }
  ...
  const scheduleImmersiveRevealPulseAfterWelcome = useCallback(() => {
    ...
    const run = () => {
      ...
      immersiveRevealPulseTimeoutRef.current = window.setTimeout(() => {
        immersiveRevealPulseTimeoutRef.current = null;
        setImmersiveRevealBtnPulse(false);
      }, IMMERSIVE_ENTER_REVEAL_PULSE_MS);
    };
  ...
  useEffect(() => {
    ...
    if (!mobileShortHeaderImmersive || !mobileViewportShort || online.userOnPause || isWaitingInRoom) {
      setMobileShortImmersiveWelcomeOpen(false);
      setImmersiveRevealBtnPulse(false);
      return;
    }
  ...
  const updateMobileShortScrollMagnet = useCallback(() => {
    ...
    if (mobileShortHeaderImmersiveRef.current) {
      const nowMag = typeof performance !== 'undefined' ? performance.now() : Date.now();
      if (nowMag < shortVhImmersiveReverseExitSuppressUntilRef.current) {
        /* см. shortVhImmersiveReverseExitSuppressUntilRef */
      } else {
    ...
    const nowForMagnet = typeof performance !== 'undefined' ? performance.now() : Date.now();
    if (
  ...
  /** Short + immersive: горизонтальный сдвиг бейджа пальцем (вне immersive не используем). */
  const onShortBadgeDragWrapPointerDown = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      if (!mobileViewportShort || !mobileShowGameInfoStrip || !mobileShortHeaderImmersive) return;
      const t = e.target as HTMLElement;
      if (t.closest('button') || t.closest('a[href]')) return;
      if (e.pointerType === 'mouse' && e.button !== 0) return;
  ...
  const onComfortHintTriggerPointerDown = useCallback((e: ReactPointerEvent<HTMLButtonElement>) => {
    if (e.pointerType === 'mouse' && e.button !== 0) return;
    comfortHintTapDownRef.current = { x: e.clientX, y: e.clientY };
  }, []);

  const onComfortHintTriggerPointerUp = useCallback(
    (e: ReactPointerEvent<HTMLButtonElement>) => {
      if (e.pointerType === 'mouse' && e.button !== 0) return;
      const dx = Math.abs(e.clientX - comfortHintTapDownRef.current.x);
  ...
  const onShortVhSouthPullTabPortalPointerOver = useCallback((e: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    flushSync(() => {
      setShortVhSouthPullTabPointerOver(true);
    });
  }, []);

  const onShortVhSouthPullTabPortalPointerLeave = useCallback(() => {
    setShortVhSouthPullTabPointerOver(false);
  }, []);

  /** Touch/pen: после отпускания нет «наведения», но pointerleave часто не приходит — снимаем «hover»-класс. */
  const clearShortVhSouthPullTabPointerOverIfNotMouse = useCallback((e: ReactPointerEvent<HTMLButtonElement>) => {
  ...
  /** Стол показывает последнюю взятку (в т.ч. фаза reveal — ещё без оверлея). */
  const lastTrickInterstitialActive =
    dealJustCompleted &&
    (lastTrickCollectingPhase === 'reveal' ||
  ...
  /** Модалка/оверлей результатов — только после паузы на столе (не на reveal/slots). */
  const dealResultsAnimOverlayActive =
    dealJustCompleted &&
    (lastTrickCollectingPhase === 'winner' ||
      lastTrickCollectingPhase === 'totals-accent' ||
      lastTrickCollectingPhase === 'collapsing');
  ...
  const lastTrickWinnerAnnounceActive =
    ...
    (lastTrickCollectingPhase === 'reveal' ||
      lastTrickCollectingPhase === 'slots' ||
      lastTrickCollectingPhase === 'winner');
  const lastTrickWinnerIdx = state?.lastCompletedTrick?.winnerIndex ?? -1;
  /** Поднять обёртку слота победителя над оверлеем результатов (как Юг z=140), иначе glow не виден. */
  const lastTrickWinnerSeatElevateStyle = (seat: number): CSSProperties =>
    lastTrickWinnerAnnounceActive && lastTrickWinnerIdx === seat
      ? { zIndex: 145, position: 'relative' as const, isolation: 'isolate' as const }
      : {};
  const mobileDealResultsInterstitialOverlayActive =
    isMobile && dealResultsAnimOverlayActive;
  ...
  useEffect(() => {
    ...
    const k = getDealType(state.dealNumber, playerCountOf(state));
  ...
  const dealResultsTimeoutsRef = useRef<{
    main?: number;
    reveal?: number;
    slots?: number;
    winner?: number;
  ...
  const pcDealResultsPortalRootRef = useRef<HTMLDivElement>(null);
  const pcDealResultsOverlayAnimRef = useRef<HTMLDivElement>(null);
  /** Защита от мгновенного «обратного» тапа по backdrop сразу после открытия модалки (моб. флэш). */
  ...
  useEffect(() => {
    ...
    if (restored) {
      ...
      let s = createGame(offlinePlayerCount === 3 ? 3 : 4, 'classical', humanName);
      s = startDeal(s);
      setLocalState(s);
    }
    ...
    // offlinePlayerCount только для новой партии (когда сейва нет); не в deps —
    // иначе HMR/ремонт App сбрасывал бы счётчик в 4 и перезапускал стол.
  ...
  const mobileSouthHandLayout = useMemo(() => {
    ...
    const lHandEligible =
      !isMobileLandscape &&
      !mobileShortHeaderImmersive &&
      mobileHandNeedsLFrame(layoutVwForHand, mobileHandLen);
  ...
  /** Якорь внешней панели Юга для fixed-ручки (портал на body — иначе режется scroll у main-wrap). */
  const mobileSouthPullTabAnchorRef = useRef<HTMLDivElement | null>(null);
  ...
  /** Вектор схлопывания оверлея к Σ: vars на корне портала (моб./ПК), иначе transform-предки ломают полёт. */
  useLayoutEffect(() => {
    ...
    const measureAndApply = (
      ...
      let oy = window.innerHeight * (isMobileLandscape ? 0.5 : 0.72);
      if (overlay) {
    ...
    if (isMobile) {
      clearVars(pcDealResultsPortalRootRef.current);
      measureAndApply(
      ...
      measureAndApply(
        pcDealResultsPortalRootRef.current,
  ...
  useLayoutEffect(() => {
    ...
    // 1) Чистый стол — успеть прочитать последнюю взятку; оверлей ещё закрыт.
    setLastTrickCollectingPhase('reveal');
    timeouts.reveal = window.setTimeout(() => {
      timeouts.reveal = undefined;
      // 2) Полёт карт к слоту победителя — оверлей ещё закрыт, иначе он перекрывает анимацию.
      setLastTrickCollectingPhase('slots');
      timeouts.slots = window.setTimeout(() => {
        timeouts.slots = undefined;
        // 3) Оверлей результатов появляется здесь (пониже); полёт уже виден/завершён.
        setLastTrickCollectingPhase('winner');
        timeouts.winner = window.setTimeout(() => {
          timeouts.winner = undefined;
          setLastTrickCollectingPhase('totals-accent');
          timeouts.totalsAccent = window.setTimeout(() => {
            timeouts.totalsAccent = undefined;
            ...
            timeouts.collapse = window.setTimeout(() => {
              ...
              if (snap && snap.dealNumber >= dealsPerMatch(playerCountOf(snap))) {
              ...
              setLastDealResultsSnapshot(
                canonicalSnap && o ? rotateStateForPlayer(snap, o.myServerIndex) : snap,
              );
  ...
  useEffect(() => {
    ...
    const allPlayed = stateRef.current?.pendingTrickCompletion?.allPlayed;
    const holdMs = trickCompleteHoldMs(allPlayed);
    const t = setTimeout(() => {
      if (isOnlinePlayPhase) {
  ...
  const renderPcTableSettings = () => {
    ...
    return (
      <div
        className={[
          'pc-table-settings',
          pcTableSettingsOpen ? 'pc-table-settings--open' : 'pc-table-settings--collapsed',
        ].join(' ')}
      >
        {!pcTableSettingsOpen ? (
          <button
          ...
          >
            ...
            <button
              ...
              aria-label={
                pcScaleWheelArmed
                  ? 'Выключить регулировку масштаба скроллом мыши'
                  : 'Включить регулировку масштаба скроллом мыши'
              }
  ...
  const openDealResultsModal = (snapshot: GameState) => {
    dealResultsOpenedAtRef.current = Date.now();
  ...
  const onDealResultsBackdropPointerUp: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    if (!dealResultsBackdropPressArmedRef.current || !isBackdrop) {
      dealResultsBackdropPressArmedRef.current = false;
      return;
    }
    dealResultsBackdropPressArmedRef.current = false;
  ...
  const renderMobileDealContractPanelButton = (opts?: { landscapeToolbar?: boolean }) => {
    ...
    const landscapeDealCls = opts?.landscapeToolbar
      ...
      : '';
    const dealType = getDealType(state.dealNumber, playerCountOf(state));
  ...
  const renderMobileGameInfoLeftSection = () => {
    ...
    /** deal-complete: не снимать plasma-экранчик — иначе бейдж схлопывается на полёте Σ и «восстанавливается» на следующей раздаче. */
    const mobileGameInfoTurnPresentation =
      !isWaitingInRoom && (state.phase === 'playing' || state.phase === 'deal-complete')
  ...
  const renderMobileNorthSlot = () => (
    ...
    >
      <OpponentSlot
        ...
        winnerPanelBlink={lastTrickWinnerAnnounceActive && lastTrickWinnerIdx === 1}
        trickWinnerHighlight={
                  (!!state.pendingTrickCompletion && state.pendingTrickCompletion.winnerIndex === 1) ||
                  (lastTrickWinnerAnnounceActive && lastTrickWinnerIdx === 1)
                }
  ...
  return (
      ...
      style={{
        ...
        ...(pcHandScaleExtraH > 0 || pcTableLiftActive || pcCardScaleMul > 1
          ? ({
              ...
              ...(pcHandScaleExtraH > 0
                ? {
                    ['--pc-hand-scale-extra-h' as string]: `${pcHandScaleExtraH}px`,
                  }
                : {}),
              ...(pcHandToPanelGap > 0
                ? {
                    ['--pc-hand-to-panel-gap' as string]: `${pcHandToPanelGap}px`,
                  }
                : {}),
              ...(pcTableLiftActive
                ? {
                    ['--game-table-up-offset' as string]: `${pcTableUpOffsetPx}px`,
                    ...(pcHandTableClearance > 0
                      ? {
                          ['--pc-hand-table-clearance' as string]: `${pcHandTableClearance}px`,
                        }
                      : {}),
                  }
                : {}),
    ...
    >
      ...
      {!isMobile && showPcNoTrumpModeTooltip && state && getDealType(state.dealNumber, playerCountOf(state)) === 'no-trump' && (
        <div
      ...
      {!isMobile && showPcDarkModeTooltip && state && getDealType(state.dealNumber, playerCountOf(state)) === 'dark' && (
        <div
          ...
          {isMobileOrTablet ? (
            ...
            >
              ...
              <div id={dealNumberExplainTitleId} style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.45, color: '#e2e8f0' }}>
                Текущая раздача — №{state?.dealNumber ?? '—'}. Всего в партии {state ? dealsPerMatch(playerCountOf(state)) : dealsPerMatch(4)} раздач.
              </div>
            </div>
          ) : (
            <div
      ...
      {isMobile &&
        ...
        !online.userOnPause && (
          <button
            ...
            onPointerDown={onComfortHintTriggerPointerDown}
            onPointerUp={onComfortHintTriggerPointerUp}
      ...
      {typeof document !== 'undefined' &&
        southShortPullTabVisible &&
        southPullTabFixedPos != null &&
        createPortal(
          <button
            type="button"
            className={
          ...
          >
            <span className="mobile-short-vh-south-pull-tab__glyphs" aria-hidden>
              <span className="mobile-short-immersive-l-handle__glyph">
                {MOBILE_SOUTH_PULL_TAB_DOT_COLORS_ROW1.map((color, i) => (
                  <span
                    key={`r1-${i}`}
                    className="mobile-short-immersive-l-handle__dot"
                    style={{ ['--dot' as string]: color }}
      ...
      {typeof document !== 'undefined' &&
        ...
        createPortal(
          ...
          <div
            ...
            aria-live="polite"
            aria-atomic="true"
            aria-label="Иммерсив: выход — кнопка с шариками сверху или жест вниз от края"
          >
            <div className="mobile-short-immersive-welcome-portal__card">
              <p className="mobile-short-immersive-welcome-portal__title">Иммерсив · без шапки</p>
              <p className="mobile-short-immersive-welcome-portal__lead">
                <span className="mobile-short-immersive-welcome-portal__hl">Кнопка с шариками</span> сверху — шапку
      ...
      <div
        ...
        style={{
          ...
          ...(!isMobile && pcTableLiftActive
            ? { transform: `translateY(-${pcTableUpOffsetPx}px)` }
            : !isMobile && useTabletPcTableTuning && isThreeSeatTable
          ...
          {isMobileLandscape && showWestSideSeat ? (
            <div
              ...
              style={{
                ...
                flex: '0 0 auto',
                flexShrink: 0,
                ...lastTrickWinnerSeatElevateStyle(2),
                ...(mobileLandscapeWePanelFixedW != null
                  ? {
              ...
              !isWaitingInRoom && (
                <ImmersiveDealContractMarquee
                  stats={{
                    ...dealContractStats,
                    dealNumber: state.dealNumber,
                    dealType: getDealType(displayState.dealNumber, playerCountOf(displayState)),
                    inBiddingPhase:
                      displayState.phase === 'bidding' || displayState.phase === 'dark-bidding',
                  }}
                  ...
                  state.lastCompletedTrick.cards.map((card, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={isTrickWinnerCard ? 'last-trick-table-card--winner' : undefined}
                        style={{
      ...
      >
        ...
        >
          ...
          >
            {rowHand.map((card, i) => {
                const marginRight = overlapPx > 0 && i < rowLen - 1 ? -overlapPx : 0;
                const humanAlreadyPlayedCurrentTrick =
                  state.phase === 'playing' &&
                  state.currentTrick.some((_, trickCardIndex) => getTrickPlayerIndex(state.trickLeaderIndex, trickCardIndex, playerCountOf(state)) === humanIdx);
                const isValidPlay =
        ...
        <div
          ...
          style={{
            ...
            ...(mobileViewportShort
              ...
              : {}),
            ...(lastTrickInterstitialActive && lastTrickWinnerIdx !== humanIdx
              ? { visibility: 'hidden' as const, pointerEvents: 'none' as const, opacity: 0 }
              : {}),
            ...(lastTrickInterstitialActive && lastTrickWinnerIdx === humanIdx
              ? { zIndex: 140, position: 'relative' as const }
              : {}),
          }}
        >
      ...
      <div className="game-info-row" style={{ ...gameInfoTopRowStyle, zIndex: 22 }}>
          ...
          >
            ...
            {(state.phase === 'playing' ||
              ...
              >
                ...
                {(state.phase === 'playing' || state.phase === 'deal-complete') && (
                  <GameInfoPlasmaTurnBlock
                    skin={gameInfoBadgeSkin}
            playerCount={playerCountOf(state)}
                    phaseStyle={gameInfoActiveBadgeStyle}
        ...
        {showPcNorthSeat ? (
          ...
          <div className="game-center-north game-center-north--three-absent" style={gameInfoNorthSlotWrapperThreeAbsent} aria-hidden />
        )}
        </div>
      <div className="game-center-spacer-top" style={centerAreaSpacerTopStyle} aria-hidden />
      <div
        ...
        style={{
          ...
          ...(dealJustCompleted
            ? {
                zIndex:
                  lastTrickWinnerAnnounceActive &&
                  (lastTrickWinnerIdx === pcWestDisplayIndex || lastTrickWinnerIdx === pcEastDisplayIndex)
                    ? 145
                    : 40,
              }
            : {}),
        ...
        {showWestSideSeat ? (
        <div
          className="game-center-west"
          style={{
            ...opponentSideWrapWestStyle,
            ...(!isMobileOrTablet ? opponentSideWrapPcGrowStyle : {}),
            ...lastTrickWinnerSeatElevateStyle(pcWestDisplayIndex),
            ...(pcSidePushPx > 0 ? { marginRight: pcSidePushPx } : {}),
          }}
        >
          <OpponentSlot state={displayState} index={pcWestDisplayIndex} position="left" inline compactMode={isMobileOrTablet}
            ...
            trickWinnerHighlight={
                        (!!state.pendingTrickCompletion && state.pendingTrickCompletion.winnerIndex === pcWestDisplayIndex) ||
                        (lastTrickWinnerAnnounceActive && lastTrickWinnerIdx === pcWestDisplayIndex)
                      }
            currentTrickLeaderHighlight={getCurrentTrickLeaderIndex(state) === pcWestDisplayIndex}
            firstBidderBadge={(state.phase === 'bidding' || state.phase === 'dark-bidding') && state.bids.some(b => b === null) && state.trickLeaderIndex === pcWestDisplayIndex}
            firstMoverBiddingHighlight={(state.phase === 'bidding' || state.phase === 'dark-bidding') && state.bids.some(b => b === null) && state.trickLeaderIndex === pcWestDisplayIndex}
            onAvatarClick={handleOpponentAvatarClick}
            onDealerBadgeClick={isMobileOrTablet ? () => setShowDealerTooltip(true) : undefined}
            offlineAiNameStyleByDifficulty={offlineAiNamePickEnabled}
          />
          ...
          <div className="game-center-west" style={{ ...opponentSideWrapWestStyle, ...(!isMobileOrTablet ? opponentSideWrapPcGrowStyle : {}), ...(pcSidePushPx > 0 ? { marginRight: pcSidePushPx } : {}) }} aria-hidden />
        )}
        <div
          className="game-center-table"
          style={{
            ...centerStyle,
        ...
        <div
          className={[
            'game-table-outer',
            useTabletPcTableTuning ? 'game-table-outer--tablet-compact-frame' : '',
          ]
            .filter(Boolean)
            .join(' ')}
          style={{
            ...tableOuterStyle,
            ...(trumpHighlightOn ? tableOuterStyleWithHighlight : {}),
            ...(useTabletPcTableTuning
              ? {
                  borderWidth: 1,
          ...
          >
            ...
            {displayState.trumpCard && (
              <DeckWithTrump
                ...
                pcCardStyles={!isMobileOrTablet}
                playerCount={displayState.players.length}
                scaleMultiplier={isMobileOrTablet ? 1 : pcCardScaleMul}
              />
            )}
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          pointerEvents: 'none',
                          zIndex: isTrickWinnerCard ? 8 : i,
                          transform: collectToWinner
                            ? getTrickSlotTransform(winnerIdx, isMobileOrTablet, isMobileLandscape, playerCountOf(state))
                            : getTrickSlotTransform(playerIdx, isMobileOrTablet, isMobileLandscape, playerCountOf(state)),
                          /* Transition всегда — иначе смена transition+transform в одном кадре глотает полёт. */
                          transition: `transform ${CARD_COLLECT_MS}ms cubic-bezier(0.22, 0.61, 0.36, 1)`,
                        }}
                      >
                        {showCardBack ? (
                          <div style={{ ...cardBackStyle, width: cardW, height: cardH }} aria-hidden />
            ...
            {state.lastCompletedTrick && (
              <button type="button" className={
                state.dealerIndex === 3 || (isThreeSeatTable && state.dealerIndex === pcEastDisplayIndex)
                  ? 'last-trick-btn last-trick-btn-left'
                  : state.dealerIndex === 1
                    ? 'last-trick-btn last-trick-btn-left-mobile-only'
                    : 'last-trick-btn'
              }
            ...
            {!isMobile && shouldShowBidPanel && bidPanelVisible && (
              <div
                className="bid-panel-pc-on-table-wrap"
                style={{
                  position: 'absolute',
                  left: 0,
                  right: 0,
        ...
        {showPcEastSeat ? (
        <div
          className="game-center-east"
          style={{
            ...opponentSideWrapEastStyle,
            ...(!isMobileOrTablet ? opponentSideWrapPcGrowStyle : {}),
            ...lastTrickWinnerSeatElevateStyle(pcEastDisplayIndex),
            ...(pcSidePushPx > 0 ? { marginLeft: pcSidePushPx } : {}),
          }}
        >
          <OpponentSlot state={displayState} index={pcEastDisplayIndex} position="right" inline compactMode={isMobileOrTablet}
            ...
            trickWinnerHighlight={
              (!!state.pendingTrickCompletion && state.pendingTrickCompletion.winnerIndex === pcEastDisplayIndex) ||
              (lastTrickWinnerAnnounceActive && lastTrickWinnerIdx === pcEastDisplayIndex)
            }
            currentTrickLeaderHighlight={getCurrentTrickLeaderIndex(state) === pcEastDisplayIndex}
            firstBidderBadge={(state.phase === 'bidding' || state.phase === 'dark-bidding') && state.bids.some(b => b === null) && state.trickLeaderIndex === pcEastDisplayIndex}
            firstMoverBiddingHighlight={(state.phase === 'bidding' || state.phase === 'dark-bidding') && state.bids.some(b => b === null) && state.trickLeaderIndex === pcEastDisplayIndex}
            onAvatarClick={handleOpponentAvatarClick}
            onDealerBadgeClick={isMobileOrTablet ? () => setShowDealerTooltip(true) : undefined}
            offlineAiNameStyleByDifficulty={offlineAiNamePickEnabled}
          />
      ...
      >
      <div
        className="game-pc-user-layer"
        style={{
        ...playerStyle,
        /* Панель Юга оставляем, если взятка ваша — иначе glow не виден. Чужую interstitial-панель прячем. */
        ...(lastTrickInterstitialActive && lastTrickWinnerIdx !== humanIdx
          ? { visibility: 'hidden' as const, pointerEvents: 'none' as const, opacity: 0 }
          : {}),
        ...(lastTrickInterstitialActive && lastTrickWinnerIdx === humanIdx
            ...
            ...(!isMobile
              ? {
                  marginBottom:
                    useTabletPcTableTuning && isThreeSeatTable
                      ? 0
                      : Math.max(2, 6 + 10 + pcHandToPanelGap),
                }
              : pcHandToPanelGap > 0
            ...
            {(() => {
              ...
              return handCards.map((card, i) => (
                <CardView
                  ...
                  forceMobileTrumpGlow={isMobileOrTablet && state.trump !== null && card.suit === state.trump && (state.phase === 'bidding' || state.phase === 'dark-bidding' || (state.phase === 'playing' && state.currentPlayerIndex === humanIdx && state.currentTrick.length === 0))}
                  mobileTrumpGlowActive={state.phase === 'bidding' || state.phase === 'dark-bidding' || (state.phase === 'playing' && state.currentPlayerIndex === humanIdx && state.currentTrick.length === 0)}
                  highlightAsValidPlay={isHumanTurn && !humanAlreadyPlayedCurrentTrick && validPlays.some(c => c.suit === card.suit && c.rank === card.rank)}
                  mobileTrumpShineBidding={(state.phase === 'bidding' || state.phase === 'dark-bidding') && state.trump !== null && card.suit === state.trump}
                  showPipZoneBorders={trumpHighlightOn}
                  pcCardStyles={!isMobileOrTablet}
          ...
          {!isMobileOrTablet ? (
              ...
              <div className="user-player-panel-pc-main">
                {(state.phase === 'bidding' || state.phase === 'dark-bidding') ? (
                  <div className="user-player-panel-pc-bidding-top-row">
                    ...
                    <div className="user-player-panel-pc-score-stack">
                      {renderPcTableSettings()}
                      <div className="user-player-panel-pc-score-dealer-col">
                        <div
            ...
            <>
              ...
              <div className="player-stats-row" style={playerStatsRowStyle}>
                <div className="user-player-panel-score-dealer-col">
                  <div
      ...
      {!isMobile && dealResultsAnimOverlayActive && createPortal(
        <div
          ref={pcDealResultsPortalRootRef}
          className={[
            'pc-deal-results-portal',
            'game-table-root',
            useTabletPcTableTuning ? 'game-table-tablet-pc' : '',
            isThreeSeatTable ? 'game-table-seats-three-pc' : '',
          ]
            .filter(Boolean)
            .join(' ')}
          style={{
        ...
        >
          {lastTrickEchoVisible && (
            <LastTrickEcho
              trick={state.lastCompletedTrick!}
              players={state.players}
              playerCount={playerCountOf(state)}
              accent={lastTrickCollectingPhase === 'winner' ? 'winner' : 'soft'}
              humanSeat={humanIdx}
            />
          )}
          <DealResultsScreen
      ...
      {isMobile && dealResultsAnimOverlayActive && createPortal(
        <div
          ...
          className={`${mobilePortalRootClass} mobile-deal-results-portal`}
          style={{
            position: 'fixed',
            inset: 0,
            pointerEvents: 'none',
...
function getOverlayPlayerPositions(
  ...
  if (playerCount === 3 && !isMobile) {
    return [
      ...
      { idx: 1, side: 'right', name: 'Восток' },
    ];
  }
  return PLAYER_POSITIONS.filter((p) => p.idx < playerCount);
}

/**
 * Мини-эхо последней взятки: читаемые мини-карты (без ПК-индексов/contentScale —
 * они раздувают пипы и дают «кашу»).
 */
function LastTrickEcho({
  trick,
  players,
  playerCount,
  accent,
  ...
  trick: NonNullable<GameState['lastCompletedTrick']>;
  ...
  const winnerName = players[trick.winnerIndex]?.name?.trim() || '—';
  const iTookIt = trick.winnerIndex === humanSeat;
  return (
    <div
      className={[
        'last-trick-echo',
        mobile ? 'last-trick-echo--mobile' : 'last-trick-echo--pc',
        accent === 'winner' ? 'last-trick-echo--winner' : 'last-trick-echo--soft',
        iTookIt ? 'last-trick-echo--mine' : '',
      ]
        .filter(Boolean)
        .join(' ')}
      aria-hidden
    >
      <div className="last-trick-echo__rail">
        {trick.cards.map((card, i) => {
          const seat = playerAtLeftFrom(trick.leaderIndex, i, playerCount);
          const isWinner = seat === trick.winnerIndex;
          return (
            <div
              key={`${card.suit}-${card.rank}-${i}`}
              className={['last-trick-echo__slot', isWinner ? 'last-trick-echo__slot--winner' : '']
                .filter(Boolean)
                .join(' ')}
            >
              <CardView
                card={card}
                compact
                scale={mobile ? 0.7 : 0.78}
                tableCardMobile
                showDesktopFaceIndices={false}
                pcCardStyles={false}
                showPipZoneBorders={false}
                doubleBorder={isWinner}
                thinBorder={!isWinner}
              />
            </div>
          );
      ...
      <div className="last-trick-echo__meta">
        <span className="last-trick-echo__kicker">{iTookIt ? 'Вы взяли:' : 'Последняя взятка:'}</span>
        <span className="last-trick-echo__winner">{iTookIt ? 'ваша!' : winnerName}</span>
      </div>
    </div>
  );
}
...
function DealResultsScreen({
  ...
  const renderMobileStickyPayoutValues = () => (
    <div
      ...
      style={{
        ...dealResultsStickyTotalsRowStyle,
        gridTemplateColumns: `${dealColumnWidth}px repeat(${players.length}, minmax(0, 1fr))`,
      }}
    >
  ...
  const _renderPanel = (idx: number) => {
    ...
    return (
      <div key={idx} style={{ ...panelStyle, ...(panelPos ?? {}) }}>
        ...
        <div style={rowStyle}>
          ...
          <span style={{ ...dealResultsValueStyle, ...getDealPointsAccentStyle(points, bid, taken, 'pill', taken) }}>
            {isExactDealPoints(points, bid, taken) && (
              <span
                style={
                  points === 5 && bid === 0 && taken === 0 ? dealResultsExactStarLegalZeroFiveStyle : undefined
                }
              >
                {points === 5 && bid === 0 && taken === 0 ? (
                  '✦'
                ) : (
                  <span style={dealResultsExactStarWrapStyle}>
                    <span style={dealResultsExactStarDefaultStyle}>✦</span>
                    <span aria-hidden style={dealResultsExactStarCenterDotStyle} />
                  </span>
                )}
              </span>
            )}
        ...
        <div style={{ ...rowStyle, borderTop: '1px solid rgba(129, 140, 248, 0.28)', marginTop: 4, paddingTop: 4 }}>
          <span style={dealResultsLabelTotalStyle}>Итого</span>
          <span style={{
            ...dealResultsValueStyle,
            ...(idx === humanIdx ? { color: '#a5b4fc' } : {}),
  ...
  const _getDealCellLabel = (dealNumber: number) => {
    const type = getDealType(dealNumber, seatCount);
    const tricks = getTricksInDeal(dealNumber, seatCount);
  ...
  /** Подписи первого столбца: вверх (без пика) + N×max + вниз + N×Б + N×Т */
  const DEAL_COLUMN_LABELS = useMemo(() => {
    const max = seatCount === 3 ? 12 : 9;
    const labels: string[] = [];
    for (let d = 1; d < max; d++) labels.push(String(d));
    for (let i = 0; i < seatCount; i++) labels.push(String(max));
    for (let d = max - 1; d >= 1; d--) labels.push(String(d));
    for (let i = 0; i < seatCount; i++) labels.push('Б');
    for (let i = 0; i < seatCount; i++) labels.push('Т');
    return labels;
  }, [seatCount]);
  const ntLabelStart = (seatCount === 3 ? 11 : 8) + seatCount + (seatCount === 3 ? 11 : 8);
  const darkLabelStart = ntLabelStart + seatCount;
  ...
  const getMobileDealLabelKind = (rowIndex: number): 'num' | 'bid' | 'dark' => {
    if (rowIndex >= ntLabelStart && rowIndex < darkLabelStart) return 'bid';
    if (rowIndex >= darkLabelStart && rowIndex < darkLabelStart + seatCount) return 'dark';
    return 'num';
  };
  ...
  const mobileTableMinWidth = compactModal
    ...
    : dealColumnWidth + players.length * (mobileBidCellWidth + mobileResultCellWidth);
  ...
  const getPlayerDealDelta = (playerIdx: number): number => latestDealRow?.points?.[playerIdx] ?? 0;
  /** Плашка (оверлей) или только цвет цифры (таблица — без фона/обводки под числом). */
  const getDealPointsAccentStyle = (
    points: number,
    bid?: number,
    taken?: number,
    surface: 'pill' | 'text' = 'pill',
    explicitTaken?: number,
  ): React.CSSProperties => {
    const takenActual =
      typeof explicitTaken === 'number' ? explicitTaken : typeof taken === 'number' ? taken : undefined;

    /** «Ровно в заказ»: фактические взятки = заказу и очки сходятся с формулой (не estimateDealPoints(bid,bid) — для 0 это ломалось). */
    const isExactOrder =
      typeof bid === 'number' &&
      typeof takenActual === 'number' &&
      bid === takenActual &&
      points === calculateDealPoints(bid, takenActual);

    /** Заказ 0 и фактически 0 взяток → «законные +5» (акцент как у плюсов); иначе +5 попадает в салат +1…+9 */
    const isLegalZeroZeroFive =
      points === 5 && typeof bid === 'number' && bid === 0 && takenActual === 0;

    /** +1…+9: кислотный салат; +10 и выше: фиолет–сирень–голубой неон (градиент). */
    const isAcidGreenBand = points >= 1 && points <= 9;
    const isNeonVioletBlueBand = points >= 10;
    ...
    if (points < 0) {
      ...
      return {
        ...
        borderRadius: 10,
        padding: '1px 9px',
        background: 'linear-gradient(135deg, rgb(127 29 29 / 0.72) 0%, rgb(185 28 28 / 0.58) 45%, rgb(136 19 55 / 0.62) 100%)',
        border: '1px solid rgb(251 113 133 / 0.75)',
        textShadow: '0 0 10px rgb(252 165 165 / 0.85), 0 0 22px rgb(248 113 113 / 0.5)',
        boxShadow: 'inset 0 1px 0 rgb(255 255 255 / 0.12), 0 0 18px rgb(239 68 68 / 0.35), 0 0 28px rgb(220 38 38 / 0.22)',
      };
    }
    ...
    if (surface === 'text') {
      ...
      if (isLegalZeroZeroFive) {
        return dealResultsLegalZeroZeroFiveTextStyle;
      }
      if (points > 0) {
        const exactBoost = isExactOrder
          ? '0 0 14px rgb(190 242 100 / 0.95), 0 0 26px rgb(163 230 53 / 0.65), 0 0 8px rgb(253 224 71 / 0.45)'
          : '0 0 12px rgb(190 242 100 / 0.92), 0 0 22px rgb(132 204 22 / 0.65), 0 0 32px rgb(101 163 13 / 0.35)';
        return {
          color: '#d9ff6b',
          WebkitTextFillColor: '#d9ff6b',
          fontWeight: isExactOrder ? 820 : 780,
          fontSize: isExactOrder ? '1.05em' : '1.02em',
          letterSpacing: '0.02em',
          textShadow: exactBoost,
        };
      }
    }
    ...
    if (surface === 'pill') {
      ...
      if (isLegalZeroZeroFive) {
        return dealResultsLegalZeroZeroFivePillStyle;
      }
      if (isAcidGreenBand) {
        return {
          color: '#0f1f0a',
          WebkitTextFillColor: '#0f1f0a',
          fontWeight: 900,
          fontSize: isExactOrder ? '1.14em' : '1.08em',
          letterSpacing: '0.02em',
          borderRadius: 10,
          padding: '1px 9px',
          background:
            'linear-gradient(145deg, rgb(217 255 107) 0%, rgb(190 242 100) 38%, rgb(132 204 22) 72%, rgb(101 163 13) 100%)',
          border: '1px solid rgb(217 249 157 / 0.95)',
          textShadow: '0 0 10px rgb(190 242 100 / 0.9), 0 1px 0 rgb(255 255 255 / 0.35)',
          boxShadow: isExactOrder
            ? 'inset 0 1px 0 rgb(255 255 255 / 0.45), 0 0 22px rgb(163 230 53 / 0.85), 0 0 36px rgb(132 204 22 / 0.45)'
            : 'inset 0 1px 0 rgb(255 255 255 / 0.38), 0 0 16px rgb(163 230 53 / 0.7), 0 0 28px rgb(101 163 13 / 0.35)',
        };
      }
      if (isNeonVioletBlueBand) {
        return {
          color: '#fef3c7',
          WebkitTextFillColor: '#fef3c7',
          fontWeight: 900,
          fontSize: isExactOrder ? '1.18em' : '1.12em',
          letterSpacing: '0.008em',
          borderRadius: 10,
          padding: '1px 10px',
          background:
            'linear-gradient(128deg, rgb(237 233 254) 0%, rgb(216 180 254) 10%, rgb(192 132 252) 22%, rgb(129 140 248) 36%, rgb(99 102 241) 48%, rgb(59 130 246) 58%, rgb(14 165 233) 72%, rgb(34 211 238) 84%, rgb(6 182 212) 92%, rgb(165 243 252) 100%)',
          border: '1px solid rgb(103 232 249 / 0.98)',
          textShadow:
            '0 1px 0 rgb(67 20 7 / 0.65), 0 1px 0 rgb(30 27 75 / 0.55), 1px 0 0 rgb(30 27 75 / 0.48), -1px 0 0 rgb(30 27 75 / 0.48), 0 -1px 0 rgb(255 255 255 / 0.26), 0 0 6px rgb(251 191 36 / 0.88), 0 0 12px rgb(34 211 238 / 0.4)',
          boxShadow: isExactOrder
            ? 'inset 0 1px 0 rgb(255 255 255 / 0.52), 0 0 24px rgb(167 139 250 / 0.55), 0 0 40px rgb(59 130 246 / 0.45), 0 0 52px rgb(34 211 238 / 0.42)'
            : 'inset 0 1px 0 rgb(255 255 255 / 0.44), 0 0 18px rgb(56 189 248 / 0.48), 0 0 32px rgb(14 165 233 / 0.38), 0 0 44px rgb(34 211 238 / 0.32)',
        };
      }
    }
    ...
    return {
      ...
      borderRadius: 9,
      padding: '1px 8px',
      background: 'linear-gradient(135deg, rgb(30 58 138 / 0.48) 0%, rgb(37 99 235 / 0.4) 55%, rgb(55 48 163 / 0.36) 100%)',
      border: '1px solid rgb(125 211 252 / 0.55)',
      textShadow: '0 0 8px rgb(147 197 253 / 0.65), 0 0 16px rgb(59 130 246 / 0.4)',
      boxShadow: 'inset 0 1px 0 rgb(255 255 255 / 0.2), 0 0 14px rgb(59 130 246 / 0.32)',
    };
  };
  const isExactDealPoints = (points: number, bid?: number, taken?: number): boolean => {
    if (typeof bid !== 'number' || typeof taken !== 'number') return false;
    if (bid !== taken) return false;
    return points === calculateDealPoints(bid, taken);
  };
  /** Цифры «Итого» по приросту раздачи — общая палитра для ПК и мобилки (на ПК поверх сохраняются плашки). */
  const getRunningTotalSmartDigitVars = (delta: number): React.CSSProperties => {
    if (delta < 0) {
      return {
        '--dr-player-total-fg': '#ff5c5c',
        '--dr-player-total-fs': '23px',
        '--dr-player-total-fw': '900',
        '--dr-player-total-ls': '0.02em',
        '--dr-player-total-lh': '1.05',
        '--dr-player-total-filter': 'none',
        /** Только тёмно-красная «гравировка» + рыжий кант — без светлых розовых/белых обводок */
        '--dr-player-total-shadow':
          '0 1px 0 rgb(127 29 29), 1px 0 0 rgb(127 29 29), -1px 0 0 rgb(127 29 29), 0 -1px 0 rgb(69 10 10), 1px 1px 0 rgb(185 28 28), -1px -1px 0 rgb(89 7 7), 0 0 2px rgb(251 146 60 / 1)',
      } as React.CSSProperties;
    }
    if (delta >= 1 && delta <= 9) {
      return {
        '--dr-player-total-fg': '#9fe868',
        '--dr-player-total-fs': '22px',
        '--dr-player-total-fw': '880',
        '--dr-player-total-ls': '0.02em',
        '--dr-player-total-lh': '1.05',
        '--dr-player-total-filter': 'none',
        '--dr-player-total-shadow':
          '0 1px 0 rgb(21 128 61 / 0.42), 0 0 6px rgb(74 222 128 / 0.72), 0 0 14px rgb(132 204 22 / 0.55), 0 0 4px rgb(34 197 94 / 0.45)',
      } as React.CSSProperties;
    }
    if (delta >= 10 && delta <= 29) {
      return {
        ...
        '--dr-player-total-fs': '23px',
        '--dr-player-total-fw': '900',
        '--dr-player-total-ls': '0.025em',
        '--dr-player-total-lh': '1',
        '--dr-player-total-filter': 'none',
        '--dr-player-total-shadow':
          '0 0 12px rgb(45 212 191 / 0.9), 0 0 24px rgb(34 211 238 / 0.65), 0 0 36px rgb(167 139 250 / 0.42), 0 0 8px rgb(139 92 246 / 0.35)',
      } as React.CSSProperties;
    }
    if (delta >= 30 && delta <= 39) {
      return {
        '--dr-player-total-fg': '#5cf0ff',
        '--dr-player-total-fs': '23px',
        '--dr-player-total-fw': '900',
        '--dr-player-total-ls': '0.03em',
        '--dr-player-total-lh': '1',
        '--dr-player-total-filter': 'none',
        '--dr-player-total-shadow':
          '0 0 14px rgb(103 232 249 / 0.95), 0 0 28px rgb(34 211 238 / 0.75), 0 0 40px rgb(56 189 248 / 0.45)',
      } as React.CSSProperties;
    }
    if (delta >= 40 && delta <= 59) {
      return {
        '--dr-player-total-fg': '#7dd3fc',
        '--dr-player-total-fs': '24px',
        '--dr-player-total-fw': '900',
        '--dr-player-total-ls': '0.03em',
        '--dr-player-total-lh': '1',
        '--dr-player-total-filter': 'none',
        '--dr-player-total-shadow':
          '0 0 14px rgb(125 211 252 / 0.95), 0 0 30px rgb(56 189 248 / 0.78), 0 0 48px rgb(99 102 241 / 0.5), 0 0 64px rgb(167 139 250 / 0.35)',
      } as React.CSSProperties;
    }
    if (delta >= 60 && delta <= 90) {
      return {
        ...
        '--dr-player-total-lh': '1',
        '--dr-player-total-filter':
          'drop-shadow(0 0 1px rgb(20 184 166 / 0.95)) drop-shadow(0 0 3px rgb(45 212 191 / 0.85)) drop-shadow(0 0 6px rgb(167 139 250 / 0.5))',
        '--dr-player-total-shadow':
          '0 0 12px rgb(255 255 255 / 0.75), 0 0 26px rgb(125 211 252 / 0.95), 0 0 44px rgb(34 211 238 / 0.8), 0 0 60px rgb(192 132 252 / 0.45), 0 0 2px rgb(13 148 136 / 0.9)',
      } as React.CSSProperties;
    }
    if (delta > 90) {
      return {
        ...
        '--dr-player-total-lh': '1.05',
        '--dr-player-total-filter': 'saturate(1.15) brightness(1.05)',
        '--dr-player-total-shadow':
          '0 0 18px rgb(255 255 255 / 0.85), 0 0 34px rgb(125 211 252 / 1), 0 0 56px rgb(34 211 238 / 0.95), 0 0 88px rgb(167 139 250 / 0.65)',
      } as React.CSSProperties;
    }
    return {
      ...
      '--dr-player-total-lh': '1.05',
      '--dr-player-total-filter': 'none',
      '--dr-player-total-shadow': '0 1px 2px rgba(15, 23, 42, 0.45)',
    } as React.CSSProperties;
  };

  /** ПК: только подложка/рамка плашки (умные цвета цифр — из getRunningTotalSmartDigitVars). */
  const getRunningTotalPcPillVars = (delta: number): React.CSSProperties => {
    if (delta < 0) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(135deg, rgb(127 29 29 / 0.52) 0%, rgb(91 33 182 / 0.45) 55%, rgb(49 46 129 / 0.5) 100%)',
        '--dr-player-total-border': '1px solid rgb(248 113 113 / 0.55)',
        '--dr-player-total-radius': '10px',
        '--dr-player-total-pad': '1px 8px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.12), inset 0 -1px 0 rgb(127 29 29 / 0.28)',
      } as React.CSSProperties;
    }
    if (delta >= 1 && delta <= 9) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(145deg, rgb(20 83 45 / 0.42) 0%, rgb(22 101 52 / 0.38) 45%, rgb(30 58 138 / 0.42) 100%)',
        '--dr-player-total-border': '1px solid rgb(134 239 172 / 0.55)',
        '--dr-player-total-radius': '9px',
        '--dr-player-total-pad': '1px 8px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.18), 0 0 14px rgb(74 222 128 / 0.28)',
      } as React.CSSProperties;
    }
    if (delta >= 10 && delta <= 29) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(140deg, rgb(88 28 135 / 0.68) 0%, rgb(99 102 241 / 0.58) 35%, rgb(37 99 235 / 0.52) 68%, rgb(30 58 138 / 0.46) 100%)',
        '--dr-player-total-border': '1px solid rgb(165 243 252 / 0.78)',
        '--dr-player-total-radius': '10px',
        '--dr-player-total-pad': '2px 9px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.26), 0 0 20px rgb(99 102 241 / 0.4)',
      } as React.CSSProperties;
    }
    if (delta >= 30 && delta <= 39) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(138deg, rgb(76 29 149 / 0.62) 0%, rgb(59 130 246 / 0.52) 50%, rgb(14 165 233 / 0.48) 100%)',
        '--dr-player-total-border': '1px solid rgb(103 232 249 / 0.72)',
        '--dr-player-total-radius': '10px',
        '--dr-player-total-pad': '2px 9px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.24), 0 0 18px rgb(56 189 248 / 0.42)',
      } as React.CSSProperties;
    }
    if (delta >= 40 && delta <= 59) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(140deg, rgb(76 29 149 / 0.78) 0%, rgb(91 33 182 / 0.74) 26%, rgb(37 99 235 / 0.62) 55%, rgb(14 165 233 / 0.55) 80%, rgb(30 58 138 / 0.5) 100%)',
        '--dr-player-total-border': '1px solid rgb(103 232 249 / 0.9)',
        '--dr-player-total-radius': '11px',
        '--dr-player-total-pad': '2px 10px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.28), 0 0 24px rgb(56 189 248 / 0.55), 0 0 40px rgb(129 140 248 / 0.42)',
      } as React.CSSProperties;
    }
    if (delta >= 60 && delta <= 90) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(140deg, rgb(76 29 149 / 0.82) 0%, rgb(91 33 182 / 0.78) 22%, rgb(37 99 235 / 0.68) 52%, rgb(14 165 233 / 0.58) 82%, rgb(30 58 138 / 0.52) 100%)',
        '--dr-player-total-border': '1px solid rgb(103 232 249 / 0.92)',
        '--dr-player-total-radius': '11px',
        '--dr-player-total-pad': '2px 10px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.3), 0 0 28px rgb(56 189 248 / 0.58), 0 0 44px rgb(129 140 248 / 0.45)',
      } as React.CSSProperties;
    }
    if (delta > 90) {
      return {
        '--dr-player-total-bg':
          'linear-gradient(135deg, rgb(76 29 149 / 0.5) 0%, rgb(37 99 235 / 0.42) 40%, rgb(14 165 233 / 0.38) 100%)',
        '--dr-player-total-border': '1px solid rgb(167 243 208 / 0.55)',
        '--dr-player-total-radius': '10px',
        '--dr-player-total-pad': '2px 10px',
        '--dr-player-total-box':
          'inset 0 1px 0 rgb(255 255 255 / 0.22), 0 0 22px rgb(56 189 248 / 0.45), 0 0 36px rgb(192 132 252 / 0.35)',
      } as React.CSSProperties;
    }
    return {
      '--dr-player-total-bg':
        'linear-gradient(145deg, rgba(71, 85, 105, 0.42) 0%, rgba(30, 41, 59, 0.48) 100%)',
      '--dr-player-total-border': '1px solid rgba(148, 163, 184, 0.35)',
      '--dr-player-total-radius': '9px',
      '--dr-player-total-pad': '2px 8px',
      '--dr-player-total-box': 'inset 0 1px 0 rgba(255, 255, 255, 0.1)',
    } as React.CSSProperties;
  };

  const getRunningTotalOverlayCssVarsPc = (delta: number): React.CSSProperties =>
    ({
  ...
  const getRunningTotalOverlayCssVars = (delta: number): React.CSSProperties =>
    isMobile ? getRunningTotalOverlayCssVarsMobile(delta) : getRunningTotalOverlayCssVarsPc(delta);

  /** Фолбэк без фазы оверлея: те же уровни delta, только цвет/свечение текста (без плашки). */
  const getHumanTotalSmartStyle = (delta: number): React.CSSProperties => {
    const v = getRunningTotalOverlayCssVars(delta) as Record<string, string | number | undefined>;
    const fg = v['--dr-player-total-fg'];
    const sh = v['--dr-player-total-shadow'];
    const fs = v['--dr-player-total-fs'];
    const fw = v['--dr-player-total-fw'];
    const ls = v['--dr-player-total-ls'];
    if (typeof fg !== 'string') return {};
    return {
      color: fg,
      WebkitTextFillColor: fg,
      fontWeight: typeof fw === 'number' ? fw : Number(fw) || 800,
      fontSize: typeof fs === 'string' ? fs : undefined,
      letterSpacing: typeof ls === 'string' ? ls : undefined,
  ...
  const getDealResultsPlayerNameCosmicStyle = (playerIndex: number, isLeader: boolean): React.CSSProperties => {
    ...
    const gradients = [
      ...
      'linear-gradient(122deg, #ecfeff 0%, #99f6e4 10%, #2dd4bf 26%, #22d3ee 42%, #0ea5e9 62%, #6366f1 84%, #c4b5fd 100%)',
      'linear-gradient(122deg, #ffffff 0%, #bae6fd 18%, #7dd3fc 36%, #38bdf8 56%, #60a5fa 74%, #818cf8 90%, #e9d5ff 100%)',
    ] as const;
    ...
    return {
      ...
      ...(!isMobileHuman && isHuman
        ? {
            backgroundImage: 'linear-gradient(122deg, #ffffff 0%, #ecfeff 10%, #a5f3fc 24%, #67e8f9 42%, #22d3ee 56%, #0ea5e9 70%, #7dd3fc 84%, #d8b4fe 100%)',
            fontWeight: 790,
            WebkitTextStroke: '0.42px rgba(224, 242, 254, 1)',
            textShadow:
              '0 1px 0 rgb(15 23 42 / 0.24), 0 0 22px rgb(103 232 249 / 0.98), 0 0 38px rgb(34 211 238 / 0.82), 0 0 56px rgb(14 165 233 / 0.62), 0 0 78px rgb(167 139 250 / 0.52)',
          }
        : {}),
    };
  };
  /** Только для `<span>` с числом: фон ячейки задаётся классами/CSS с `!important` — акцент на td не виден. */
  const getDealPointsTextSpanStyle = (row: DealResult, playerIdx: number): React.CSSProperties => {
    const points = row.points[playerIdx];
    const bid = row.bids?.[playerIdx];
  ...
  const isOverlayPanels = variant === 'overlay';
  ...
  const overlayDealRowStyle: React.CSSProperties =
    isMobile && isOverlayPanels
      ...
      : isOverlayPCLayout
        ? { ...dealResultsRowStyle, fontSize: 15, gap: 14 }
        : dealResultsRowStyle;
  const overlayLabelStyle: React.CSSProperties =
    isMobile && isOverlayPanels
      ? dealResultsLabelStyleMobileOverlay
      : isOverlayPCLayout
        ? { ...dealResultsLabelStyle, fontSize: 14 }
        : dealResultsLabelStyle;
  const overlayLabelTotalStyle: React.CSSProperties =
    isMobile && isOverlayPanels
      ? dealResultsLabelTotalStyleMobileOverlay
      : isOverlayPCLayout
        ? { ...dealResultsLabelTotalStyle, fontSize: 15 }
        : dealResultsLabelTotalStyle;
  const overlayValueStyle: React.CSSProperties =
    isMobile && isOverlayPanels
      ? dealResultsValueStyleMobileOverlay
      : isOverlayPCLayout
        ? { ...dealResultsValueStyle, fontSize: 18 }
        : dealResultsValueStyle;
  const toggleLeaderBadgeTooltip = (anchorEl?: HTMLElement) => {
    ...
    const placeAbove = preferredTop + tooltipHeight > window.innerHeight;
    const top = placeAbove ? Math.max(12, rect.top - tooltipHeight - 8) : preferredTop;
    const anchorCenter = rect.left + rect.width / 2;
    const arrowX = Math.max(18, Math.min(tooltipWidth - 18, anchorCenter - left));
    setLeaderBadgeTooltipPos({ left, top, placement: placeAbove ? 'above' : 'below', arrowX });
    setLeaderBadgeTooltipOpen(true);
  };
  useEffect(() => {
    if (!leaderBadgeTooltipOpen) return;
    const closeTooltip = () => setLeaderBadgeTooltipOpen(false);
    const autoHideTimer = window.setTimeout(closeTooltip, 8500);
    const onPointerDown = (event: PointerEvent) => {
      const target = event.target as HTMLElement | null;
      if (!target) return;
      if (target.closest('.deal-results-name-rank-badge')) return;
      if (target.closest('.deal-results-name-rank-badge-tooltip-floating')) return;
      closeTooltip();
    };
  ...
  const renderLandscapeDockColgroup = () => (
    <colgroup>
      ...
      {[0, 1, 2, 3].slice(0, players.length).map((i) => (
        <Fragment key={i}>
        ...
        <tbody>
          <tr className="deal-results-dock-landscape-row deal-results-dock-landscape-row--scores">
            ...
            <td colSpan={players.length * 2} className="deal-results-dock-landscape-data-cell">
              <div
                className="deal-results-dock-landscape-player-grid"
  ...
  const mobileDealResultsBodyTable = (
                <table className="deal-results-table-body-pc" style={mobileTableStyle}>
                  ...
                  <tbody>
                    {Array.from({ length: dealsPerMatch(seatCount) }, (_, i) => i + 1).map((dealNum, rowIndex) => {
                      const row = dealHistory.find((r) => r.dealNumber === dealNum);
                      const isLastBodyDealRow = dealNum === dealsPerMatch(seatCount);
                      return (
                        <tr key={dealNum}>
                          <td
  ...
  return (
    <div
      ...
      className={
        variant === 'overlay'
          ? [
              'deal-results-overlay-animation',
              seatCount === 3 ? 'deal-results-overlay-animation--three' : '',
              isCollapsing ? 'deal-results-overlay-animation--collapsing' : '',
            ]
              .filter(Boolean)
              .join(' ')
          : undefined
      }
      {...(variant === 'overlay' && overlayAccentPhase ? ({ 'data-overlay-accent-phase': overlayAccentPhase } as const) : {})}
      style={{
        ...baseStyle,
        ...(isCollapsing
          ? {
              ...(isMobile ? dealResultsCollapsingStyleMobile : dealResultsCollapsingStyle),
              /* animation управляет transform — не держим transform:none из base */
              transform: undefined,
              overflow: 'visible',
              /* ниже last-trick-echo (160), выше стола */
              zIndex: 110,
            }
          : {}),
      }}
    ...
    >
                                ...
                                <th
                                  ...
                                  className={
                                    [
                                      ...
                                      i === humanIdx && 'deal-results-cell-human',
                                      isLeader && 'deal-results-column-leader deal-results-column-leader-r',
                                      isLeader && 'deal-results-name-cell--leader',
                                    ]
                                      .filter(Boolean)
                                      .join(' ') || undefined
                                  }
                                  style={{ ...dealResultsTableThStyleMobile, ...dealResultsTableThNameStyle }}
                                  title={p.name}
                                >
                                  {isLeader && (
                                    <span
                                      className={`deal-results-name-rank-badge${leaderBadgeTooltipOpen ? ' deal-results-name-rank-badge--tooltip-open' : ''}`}
                                      tabIndex={0}
                                      aria-label="Космический бейдж лидера по очкам"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        toggleLeaderBadgeTooltip(e.currentTarget);
                                      }}
                                    >
                                      <span className="deal-results-name-rank-badge-surface" aria-hidden />
                                      <span className="deal-results-name-rank-badge-digit">1</span>
                                    </span>
                                  )}
                                  <span
                            ...
                            {players.map((p, i) => {
                              ...
                              return (
                                <th
                                  ...
                                  colSpan={2}
                                  className={
                                    [i === humanIdx && 'deal-results-cell-human', isLeader && 'deal-results-column-leader deal-results-column-leader-r', isLeader && 'deal-results-name-cell--leader'].filter(Boolean).join(' ') || undefined
                                  }
                                  style={{ ...dealResultsTableThStyle, ...dealResultsTableThNameStyle }}
                                  title={p.name}
                                >
                                  {isLeader && (
                                    <span
                                  ...
                                  ? players.map((_, i) => {
                                      ...
                                      return (
                                        <Fragment key={i}>
                                          ...
                                          <td className={[isLeader && 'deal-results-column-leader', isHuman && 'deal-results-cell-human'].filter(Boolean).join(' ') || undefined} style={dealResultsTableTdResultStyle}>
                                            {isExactDealPoints(row.points[i], rowBid, rowTaken) &&
                                              (starLegalZeroFive ? (
      ...
      {getOverlayPlayerPositions(players.length, isMobile).map(({ idx, side }) => {
        ...
        const legalZeroFiveExactStar =
          isExactDealPoints(points, bid, taken) && points === 5 && bid === 0 && taken === 0;
        ...
        return (
          <div
            ...
            style={{
              ...
              /* ПК-оверлей: абсолютные panelPos ломают grid (съезд/вылет); позиция — только CSS. */
              ...(!(isMobile && isOverlayPanels) && !isOverlayPCLayout ? panelPos : {}),
            }}
          >
            {isMobile && isOverlayPanels ? (
              <div
      ...
      {leaderBadgeTooltipOpen && leaderBadgeTooltipPos && createPortal(
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          <span className="deal-results-name-rank-badge-tooltip-title-wrap">
            <span className="deal-results-name-rank-badge-tooltip-badge-mini" aria-hidden />
            <span className="deal-results-name-rank-badge-tooltip-title">Космический лидер</span>
          </span>
          <span className="deal-results-name-rank-badge-tooltip-text">Максимум очков в текущем рейтинге.</span>
          <button
            type="button"
...
function TrickSlotsDisplay({
    ...
    let scaleDown =
      variant === 'player' && playerMobileWideTricks
        ...
        : variant === 'player' && bid > 6
          ? Math.min(1, 6 / bid)
          : 1;
    /** Мобильная полоса без «Заказ:» (Север/Запад) — ширина слота; сжатие с 5+ слотов */
    let opponentScaleDown =
      variant === 'opponent' && !eastMobileTricks && bid > 4 && !opponentMobileHideOrderLabel
        ? Math.min(1, 4 / bid)
        : 1;
  ...
  const effectiveSlotSize =
    pcSlotScaleDown < 1
      ? variant === 'opponent'
        ? {
            w: Math.max(12, Math.round(44 * pcSlotScaleDown)),
            h: Math.max(20, Math.round(62 * pcSlotScaleDown)),
          }
...
/** Бейдж «Ровно» на ПК: только активная игра (розыгрыш / взятка) и все заказы выставлены */
function shouldShowPcOrderOnHandExactBadge(state: GameState): boolean {
  ...
  const n = playerCountOf(state);
  return state.bids.length === n && state.bids.every((b) => b != null);
}
  ...
  return (
    <div
      ...
      style={{
        ...
        ...frameStyle,
        overflow: 'visible',
        /* Зелёный pulse важнее cyan winnerPanelBlink — не ставим конфликтующую animation. */
        ...(winnerPanelBlink && !trickWinnerHighlight
          ? { animation: 'winnerPanelBlink 0.5s ease-in-out 2' }
          : {}),
        ...
        ...(dealerOpponentMobilePanelPlaying
          ...
          : {}),
        /* Взятка — последней: иначе glow сдающего/хода затирает зелёный. */
        ...(trickWinnerHighlight
          ? {
              ...trickWinnerGlowStyle,
              ...
              boxShadow: [
                frameStyle?.boxShadow ?? opponentSlotStyle.boxShadow,
                trickWinnerGlowStyle.boxShadow,
              ]
                .filter(Boolean)
                .join(', '),
            }
...
function DeckWithTrump({
  ...
  playerCount?: number;
  /** ПК: пользовательский масштаб карт на сукне (1 = без boost). */
  scaleMultiplier?: number;
}) {
  const cardsUnderTrump = getDeckCardsUnderTrump(tricksInDeal, playerCount === 3 ? 3 : 4);
  ...
  const deckScale = (compactTable ? 1.18 / 1.2 : 1.18) * (compactTable ? 1 : scaleMultiplier);
...
const playerSpacerStyle: React.CSSProperties = {
  height:
    'calc(var(--game-player-area-height, 260px) + var(--pc-hand-scale-extra-h, 0px) + var(--pc-hand-table-clearance, 0px) + var(--game-player-bottom, 34px) + var(--pc-south-layer-gap, 8px))',
  flexShrink: 0,
};
...
/** ПК · 3 игрока: место Севера не резервируем — стол ближе к шапке. */
const gameInfoNorthSlotWrapperThreeAbsent: React.CSSProperties = {
  width: 0,
  flex: '0 0 0',
  height: 0,
  overflow: 'hidden',
...
const dealResultsOverlayStyle: React.CSSProperties = {
  ...
  boxShadow: [
    ...
    '0 0 40px rgba(91, 33, 182, 0.28)',
    '0 0 80px rgba(99, 102, 241, 0.14)',
  ].join(', '),
  animation: 'dealResultsFadeIn 0.5s ease-out',
};

/** Моб. interstitial: в потоке flex-портала у низа, высота по контенту. */
const dealResultsOverlayStyleMobile: React.CSSProperties = {
  position: 'relative',
  top: 'auto',
  bottom: 'auto',
  left: 'auto',
...
/** Мобильный оверлей итогов раздачи: схлопывание к Σ в панели «Заказывает». */
...
const _dealResultsChartWrapStyle: React.CSSProperties = {
  ...
  boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.06), 0 0 14px rgba(91, 33, 182, 0.22)',
};
...
/** Цвет «О» (очки): холодный сиренево-голубой, без бирюзы */
const DEAL_RESULTS_O_COLOR = '#a5b4fc';
/** Знак «+» — кислотно-салатовый неон для всех положительных значений. */
const dealResultsPositiveSignStyle: React.CSSProperties = {
  color: '#d9ff6b',
  WebkitTextFillColor: '#d9ff6b',
  fontWeight: 900,
  fontSize: '1.28em',
  ...
  top: '0.03em',
  textShadow:
    '0 1px 0 rgb(77 124 15 / 0.58), 0 -1px 0 rgb(54 83 20 / 0.52), 0 0 2px rgb(190 242 100 / 0.98), 0 0 10px rgb(163 230 53 / 0.82)',
};
/** +5 за заказ 0 взяток и ровно 0 взяток — как плюсики (кислотная бирюза), не как салат +1…+9 */
const dealResultsLegalZeroZeroFiveTextStyle: React.CSSProperties = {
  color: '#22d3ee',
  ...
  textShadow:
    '0 1px 0 rgb(94 234 212 / 0.95), 0 -1px 0 rgb(13 148 136 / 0.65), 0 0 1px rgb(34 211 238 / 1), 0 0 2px rgb(45 212 191 / 0.85)',
};
const dealResultsLegalZeroZeroFivePillStyle: React.CSSProperties = {
  color: '#052025',
  WebkitTextFillColor: '#052025',
  fontWeight: 900,
  fontSize: '1.12em',
  letterSpacing: '0.015em',
  borderRadius: 10,
  ...
  boxShadow: 'inset 0 1px 0 rgb(255 255 255 / 0.5), 0 0 14px rgb(34 211 238 / 0.42)',
};
/** ✦ «законные +5»: заливка бирюза/циан; ореол — сиреневый неон (усилен для читаемости на фоне) */
const dealResultsExactStarLegalZeroFiveStyle: React.CSSProperties = {
  marginRight: 4,
  display: 'inline-block',
  ...
  WebkitTextFillColor: 'transparent',
  backgroundImage:
    'linear-gradient(158deg, #7c3aed 0%, #06b6d4 14%, #0891b2 22%, #22d3ee 44%, #0ea5e9 48%, #38bdf8 49%, #22d3ee 51%, #14b8a6 68%, #2dd4bf 76%, #06b6d4 86%, #a855f7 94%, #c4b5fd 100%)',
  WebkitBackgroundClip: 'text',
  backgroundClip: 'text',
  textShadow:
    '0 0 2px rgb(224 242 254 / 1), 0 0 5px rgb(186 230 253 / 1), 0 0 14px rgb(192 132 252 / 0.98), 0 0 22px rgb(168 85 247 / 0.94), 0 0 30px rgb(147 51 234 / 0.85), 0 0 38px rgb(126 34 206 / 0.68)',
  filter:
    'drop-shadow(0 0 3px rgb(192 132 252 / 1)) drop-shadow(0 0 9px rgb(168 85 247 / 0.96)) drop-shadow(0 0 18px rgb(147 51 234 / 0.88)) drop-shadow(0 0 26px rgb(126 34 206 / 0.68))',
};
/** ✦ обычное: reset-стиль — три явно читаемых цвета + тонкий cyan-кант + короткие блики */
const dealResultsExactStarDefaultStyle: React.CSSProperties = {
  marginRight: 0,
  display: 'inline-block',
  fontWeight: 900,
  fontSize: '1.2em',
  ...
  WebkitTextStroke: '0.22px rgb(34 211 238 / 1)',
  backgroundImage:
    'radial-gradient(circle, #00ffd5 0%, #2dd4bf 36%, rgb(20 184 166 / 0) 68%), radial-gradient(ellipse 98% 108% at 50% 45%, #faf5ff 0%, #f3e8ff 2%, #e9d5ff 9%, #c4b5fd 18%, #38bdf8 19%, #22d3ee 44%, #6d28d9 45%, #7c3aed 72%, #d946ef 73%, #e879f9 86%, #38bdf8 87%, #22d3ee 100%)',
  backgroundSize: '16% 16%, 100% 100%',
  backgroundPosition: '50% 45%, center',
  backgroundRepeat: 'no-repeat, no-repeat',
  WebkitBackgroundClip: 'text',
  backgroundClip: 'text',
  textShadow:
    '0 -1px 0 rgb(240 249 255 / 0.7), 0 0 1px rgb(250 245 255 / 0.38), 0 0 1px rgb(34 211 238 / 1), 0 0 2px rgb(139 92 246 / 0.9), 0 0 2px rgb(232 121 249 / 0.68)',
  filter: 'none',
};
const dealResultsExactStarWrapStyle: React.CSSProperties = {
  position: 'relative',
  display: 'inline-block',
  width: '1.2em',
  height: '1.2em',
...
const dealResultsExactStarCenterDotStyle: React.CSSProperties = {
  position: 'absolute',
  left: '48%',
  top: '50%',
  transform: 'translate(-50%, -50%)',
  ...
  boxShadow:
    '0 0 3px rgb(255 255 255 / 1), 0 0 10px rgb(0 207 255 / 1), 0 0 16px rgb(34 211 238 / 1)',
  zIndex: 10,
  pointerEvents: 'none',
};
/** ✦ у «Итого» при крупном плюсе — кислотный зелёный + золотисто-голубой ореол */
const dealResultsExactStarUltraTotalStyle: React.CSSProperties = {
  marginRight: 4,
  display: 'inline-block',
  fontWeight: 900,
  fontSize: '1.12em',
  ...
  WebkitTextFillColor: '#d9ff6b',
  textShadow:
    '0 0 12px rgb(190 242 100 / 1), 0 0 26px rgb(250 204 21 / 0.95), 0 0 44px rgb(34 211 238 / 1), 0 0 18px rgb(253 224 71 / 0.9)',
  filter:
    'drop-shadow(0 0 7px rgb(163 230 53 / 1)) drop-shadow(0 0 16px rgb(34 211 238 / 0.95)) drop-shadow(0 0 12px rgb(250 204 21 / 0.88))',
};
...
const dealResultsTableScrollHintChevronStyle: React.CSSProperties = {
  ...
  fontSize: 16,
  fontWeight: 700,
  boxShadow: '0 0 14px rgba(139, 92, 246, 0.32), inset 0 0 8px rgba(91, 33, 182, 0.12)',
};
...
const dealResultsTableGlowTopStyle: React.CSSProperties = {
  ...
  background: 'linear-gradient(to bottom, rgba(147, 51, 234, 0.42) 0%, rgba(99, 102, 241, 0.26) 45%, transparent 100%)',
  boxShadow:
    '0 4px 22px rgba(139, 92, 246, 0.35), 0 0 16px rgba(99, 102, 241, 0.22), inset 0 1px 0 rgba(167, 139, 250, 0.45)',
};
/** Мобильная таблица: верхний «лазерный» край холодной палитры без тёплых примесей. */
const dealResultsTableGlowTopStyleMobile: React.CSSProperties = {
  ...dealResultsTableGlowTopStyle,
...
const dealResultsTableGlowBottomStyle: React.CSSProperties = {
  ...
  background: 'linear-gradient(to top, rgba(124, 58, 237, 0.48) 0%, rgba(99, 102, 241, 0.28) 50%, transparent 100%)',
  boxShadow:
    '0 -5px 24px rgba(139, 92, 246, 0.36), 0 0 20px rgba(99, 102, 241, 0.24), inset 0 -1px 0 rgba(167, 139, 250, 0.42)',
};
/** Мобильная таблица: нижняя «космическая» кромка в cyan/indigo спектре. */
const dealResultsTableGlowBottomStyleMobile: React.CSSProperties = {
  ...dealResultsTableGlowBottomStyle,
  background: 'linear-gradient(to top, rgba(14, 165, 233, 0.34) 0%, rgba(99, 102, 241, 0.24) 44%, rgba(56, 189, 248, 0.2) 62%, transparent 100%)',
  boxShadow:
    '0 -5px 24px rgba(14, 165, 233, 0.34), 0 0 20px rgba(59, 130, 246, 0.26), inset 0 -1px 0 rgba(125, 211, 252, 0.4)',
};
...
const dealResultsTableWindowStyle: React.CSSProperties = {
  ...
  boxShadow: [
    'inset 0 -20px 28px -8px rgba(15, 23, 42, 0.55)',
    '0 0 0 1px rgba(139, 92, 246, 0.22)',
    '0 4px 22px rgba(30, 20, 60, 0.45)',
    'inset 0 14px 22px -8px rgba(99, 102, 241, 0.12)',
    'inset 0 -18px 24px -8px rgba(76, 29, 149, 0.18)',
    'inset 0 0 36px -8px rgba(91, 33, 182, 0.1)',
  ].join(', '),
  background: 'linear-gradient(180deg, rgba(15, 23, 42, 0.98) 0%, rgba(15, 23, 42, 0.99) 100%)',
  padding: 1,
};
/** Мобильная модалка: более «космическое» окно таблицы (холодный неон + глубина). */
const dealResultsTableWindowStyleMobile: React.CSSProperties = {
  ...dealResultsTableWindowStyle,
  border: '1px solid rgba(56, 189, 248, 0.42)',
  boxShadow: [
    'inset 0 -24px 32px -10px rgba(2, 6, 23, 0.62)',
    'inset 0 14px 24px -8px rgba(14, 165, 233, 0.16)',
    'inset 0 -20px 30px -12px rgba(59, 130, 246, 0.2)',
    '0 0 0 1px rgba(34, 211, 238, 0.2)',
    '0 6px 24px rgba(8, 47, 73, 0.46)',
    '0 0 22px rgba(59, 130, 246, 0.26)',
  ].join(', '),
  background: 'linear-gradient(180deg, rgba(2, 6, 23, 0.985) 0%, rgba(15, 23, 42, 0.995) 100%)',
};
...
const dealResultsTableThStyleMobile: React.CSSProperties = {
  ...dealResultsTableThStyle,
  borderBottom: '2px solid rgba(56, 189, 248, 0.48)',
  background: 'rgba(2, 6, 23, 0.94)',
  boxShadow: 'inset 0 -1px 0 rgba(125, 211, 252, 0.14)',
};
...
const dealResultsTableTdStyleMobile: React.CSSProperties = {
...
const dealResultsTableTdResultStyleMobile: React.CSSProperties = {
  ...dealResultsTableTdResultStyle,
  ...dealResultsTableTdStyleMobile,
...
const dealResultsTableThBidStyle: React.CSSProperties = {
  ...
  borderRight: '2px solid rgba(129, 140, 248, 0.55)',
  boxShadow: 'inset 0 0 10px rgba(91, 33, 182, 0.12), inset 0 1px 0 rgba(167, 139, 250, 0.2)',
};
...
const dealResultsTableThResultStyle: React.CSSProperties = {
  ...
  boxShadow: 'inset 0 0 10px rgba(55, 48, 163, 0.2), inset 0 1px 0 rgba(165, 180, 252, 0.16)',
};
const dealResultsTableThResultStyleMobile: React.CSSProperties = {
  ...dealResultsTableThResultStyle,
  ...dealResultsTableThStyleMobile,
  background: 'rgba(30, 58, 138, 0.32)',
  borderLeft: '2px solid rgba(56, 189, 248, 0.42)',
  borderRight: '2px solid rgba(56, 189, 248, 0.42)',
  boxShadow: 'inset 0 0 12px rgba(59, 130, 246, 0.18), inset 0 1px 0 rgba(125, 211, 252, 0.18)',
};
...
const dealResultsPlayerNameTextBaseStyle: React.CSSProperties = {
  ...dealResultsTableThNameTextStyle,
  display: 'inline-block',
  fontFamily: '"Manrope", "Plus Jakarta Sans", "Inter", "SF Pro Display", "Segoe UI Variable Display", "Segoe UI", system-ui, sans-serif',
  fontWeight: 740,
  fontSize: '1.08em',
  letterSpacing: '0.01em',
  color: 'transparent',
  ...
  WebkitBackgroundClip: 'text',
  backgroundClip: 'text',
  WebkitTextStroke: '0.24px rgba(224, 242, 254, 0.62)',
  textShadow:
    '0 1px 0 rgb(15 23 42 / 0.38), 0 0 2px rgb(255 255 255 / 0.36), 0 0 12px rgb(125 211 252 / 0.34)',
};
/** Закреплённый низ модалки (моб): полупрозрачная «glass» плашка с итогами игроков. */
const dealResultsStickyTotalsDockStyle: React.CSSProperties = {
  position: 'absolute',
  left: 1,
  right: 1,
  bottom: 0,
...
const _dealResultsChartBarFillStyle: React.CSSProperties = {
  height: '100%',
  borderRadius: 3,
  background: 'linear-gradient(90deg, rgba(99, 102, 241, 0.55) 0%, rgba(139, 92, 246, 0.75) 100%)',
  transition: 'width 0.5s ease-out',
};

const _dealResultsChartBarLeaderStyle: React.CSSProperties = {
  background: 'linear-gradient(90deg, rgba(167, 139, 250, 0.85) 0%, rgba(129, 140, 248, 0.95) 50%, rgba(99, 102, 241, 0.9) 100%)',
...
const dealResultsButtonStyle: React.CSSProperties = {
  ...
  background: 'linear-gradient(135deg, rgba(76, 29, 149, 0.92) 0%, rgba(67, 56, 202, 0.88) 100%)',
  boxShadow: '0 0 14px rgba(91, 33, 182, 0.45), inset 0 1px 0 rgba(255,255,255,0.2)',
  cursor: 'pointer',
  display: 'flex',
  alignItems: 'center',
  justifyContent: 'center',
...
const dealResultsPanelStyle: React.CSSProperties = {
  ...
  boxShadow: [
    '0 0 0 1px rgba(91, 33, 182, 0.22)',
    '0 0 16px rgba(76, 29, 149, 0.2)',
...
/** ПК-оверлей результатов раздачи: шире панели, длинные имена с переносом */
...
const dealResultsValueLeaderStyle: React.CSSProperties = {
  ...
  background:
    'linear-gradient(135deg, rgba(76, 29, 149, 0.82) 0%, rgba(67, 56, 202, 0.88) 50%, rgba(49, 46, 129, 0.85) 100%)',
  padding: '2px 8px',
  borderRadius: 6,
  boxShadow: 'inset 0 0 14px rgba(124, 58, 237, 0.22), 0 0 12px rgba(91, 33, 182, 0.35)',
};
...
const playerStyle: React.CSSProperties = {
  ...
  left: 0,
  right: 0,
  maxHeight: 'calc(var(--game-player-area-height, 260px) + var(--pc-hand-scale-extra-h, 0px))',

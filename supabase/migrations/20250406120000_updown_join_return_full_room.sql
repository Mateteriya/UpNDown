-- Одним ответом отдаём полную строку game_rooms — без второго REST getRoom (экономия ~1 RTT и до 38 с ожидания).
...
comment on function public.updown_join_waiting_room(text, uuid, text, text, text) is
  'Атомарный вход в waiting; успех: room (полная строка) + my_slot_index.';

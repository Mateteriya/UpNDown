-- Завершение партии: вставка в online_party_outcomes не должна откатывать UPDATE (иначе 400/500 и кнопка «Завершить» не работает).
-- Снимок: r.game_state как jsonb без to_jsonb(r.game_state) — double-encoding в редких схемах.
...
begin
  ...
  if ch = 'finish' then
    ...
    exception when others then
      -- История — не суть: партия всё равно должна завершиться, если вставка/таблица/права мешают
      null;
    end;
    ...
    -- Колонка player_slots: json. Обе ветки CASE — jsonb, затем единая присвоение ::json (иначе «json vs jsonb»).

-- Деталь архива партии: все слоты + подсказка аккаунта (local-part email).
-- Без %rowtype — устойчивее к разному набору колонок matches.
...
exception
  when undefined_column then
    -- Старая схема без is_offline / settlement_mode / buy_in / chips / deal_history
    select jsonb_build_object(
      'id', m.id,
      'code', m.code,
      'finished_at', m.finished_at,
...
comment on function public.updown_get_match_archive_detail(uuid) is
  'Архив: матч + все слоты + account_hint (email local-part); только участник партии.';

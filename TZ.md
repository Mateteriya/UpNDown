...
#### 2.1.4. Итог партии: очки и фишки

После всех раздач цикла у каждого игрока есть **сумма очков** по правилам §2.1.2 (`dealHistory`, поле `score`). Поверх этой суммы в интерфейсе показываются **фишки** — условные «деньги» / интерес итога; они **не заменяют** очки в таблице раздач.

| Режим | ID | Смысл |
|-------|-----|--------|
| Только очки | `points_only` | Без фишек (legacy / отладка) |
| **Точный заказ** | `accuracy_bonus` | Бонус за долю раздач с точным заказом; штраф за отставание от лидера по очкам |
| Середина стола | `vs_average` | Фишки = отклонение от среднего по очкам; сумма фишек ≈ 0 |
| Банк (турнир) | `prize_pool` | Взнос в банк, распределение по местам (50/30/15/5 при 4 игроках; 60/30/10 при 3) |

**Рейтинг и победа в статистике** — по **очкам** партии (максимум `score`), как и раньше. **Фишки** не меняют `updateLocalRating` и не определяют «победу» в рейтинге.

**Офлайн UX:** режим фишек **не выбирается до старта** партии. Игрок переключает **«Точный заказ» | «Середина стола»** в таблице результатов (Σ) и на экране «Партия завершена»; выбор сохраняется в `localStorage` (`updown_results_chip_view`). Завершённые партии пишутся в локальный **журнал** по `profileId` (до 90 дней / 1500 записей на устройстве) — это не лимит счётчика «игр сыграно» в рейтинге.

**Рейтинг vs журнал:** счётчик побед/игр (`updown_rating_<profileId>`) **без верхней границы**. `profileId` — uuid в профиле; при входе в аккаунт синхронизируется с Supabase (`profiles.profile_id`), чтобы на другом устройстве под тем же логином подтянуть тот же id. Полная история на всех устройствах — через **вход в аккаунт** и запись матча в облако (`record_offline_match`), не через копирование localStorage.

**Онлайн (позже):** режим итога задаётся при создании комнаты (`settlement_mode`, для банка — `buy_in`); все клиенты считают `computePartySettlement` из общего `dealHistory`.

Реализация: `src/game/partySettlement.ts`, план — `docs/PARTY-SETTLEMENT-PLAN.md`.
